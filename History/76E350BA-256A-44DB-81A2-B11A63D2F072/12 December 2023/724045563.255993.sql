...
DROP TABLE IF EXISTS hc.contact_to_emails_temp;
CREATE TABLE hc.contact_to_emails_temp AS (
	SELECT * FROM split_additional_emails
	UNION
	SELECT * FROM primary_emails;
);
DROP TABLE IF EXISTS hc.contact_to_emails;
ALTER TABLE hc.contact_to_emails_temp RENAME TO contact_to_emails;