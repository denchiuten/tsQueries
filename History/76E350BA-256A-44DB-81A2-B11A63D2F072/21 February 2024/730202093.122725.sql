SELECT
	...
	SUM(COUNT(DISTINCT pm.member_id)) OVER (PARTITION BY p.id),
	SUM(COALESCE(i.estimate, 1)) / SUM(COUNT(DISTINCT pm.member_id)) OVER (PARTITION BY p.id) AS points_completed_per_member
