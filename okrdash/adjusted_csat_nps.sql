SELECT DISTINCT
	fb.record_id,
	fb.survey_name,
	CASE WHEN fb.date ~'^[0-9]+$' 
		THEN TIMESTAMP 'epoch' + (fb.date / 1000) * INTERVAL '1 second'
		ELSE fb.date::TIMESTAMP
	END AS date_submitted,
	
	CASE WHEN fb.object_create_date_time ~'^[0-9]+$' 
		THEN TIMESTAMP 'epoch' + (fb.object_create_date_time / 1000) * INTERVAL '1 second'
		ELSE fb.object_create_date_time::TIMESTAMP
	END AS date_created,

	CASE WHEN fb.object_last_modified_date_time ~'^[0-9]+$' 
		THEN TIMESTAMP 'epoch' + (fb.object_last_modified_date_time / 1000) * INTERVAL '1 second'
		ELSE fb.object_last_modified_date_time::TIMESTAMP
	...
	fb.overall_rating AS overall_rating,
